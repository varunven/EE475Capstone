...
def event_listener(obj_recog_worker, facial_recog_worker, obj_detect_worker):
	while True:
		...
		elif event.service_name == 'object_recognition-settings':
			obj_recog_worker.handle_event(event)
		elif event.service_name == 'object_detection-settings':
...
def main():
  ...
  face_recognition_service = FacialRecognitionWorker(audio_controller)

  sidewalk_detection_service = SidewalkDetectionWorker(YOLO_SERVER_LINK)
  
  event_listener_thread = threading.Thread(target=event_listener, args=(obj_recognition_service, face_recognition_service, obj_detection_service))
